...
public class QiniuUtil implements StoreService {
    ...
    public void deleteFile(List<String> files) {
        ...
        try {
            ...
            for (int i = 0; i < keyList.length; i++) {
                ...
                if (status.code == 200) {
                    log.info("文件删除成功：" + key);
                    resourcePow.lambdaUpdate().eq(ResourcePO::getPath, downloadUrl + key).remove();
                } else {
    ...
    public void saveFileInfo() {
        List<ResourcePO> resourcePOList = resourcePow.lambdaQuery().select(ResourcePO::getPath).list();
        List<String> paths = resourcePOList.stream().map(ResourcePO::getPath).collect(Collectors.toList());
        ...
        List<ResourcePO> resources = new ArrayList<>();
        ...
        if (!CollectionUtils.isEmpty(resources)) {
            resourcePow.saveBatch(resources);
